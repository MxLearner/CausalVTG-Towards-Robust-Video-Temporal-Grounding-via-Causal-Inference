...
def compute_mr_ap(submission,
    ...
    for d in submission:
        pred_windows = d["pred_relevant_windows"][:max_pred_windows] \
            if max_pred_windows is not None else d["pred_relevant_windows"]  # 会接近40个预测，之前的函数已经根据得分降序排序过了
        qid = d["qid"]
...
def get_data_by_range(submission, ground_truth, len_range):
    ...
    if submission_in_range == ground_truth_in_range == []: # NOTE:  为什么等于空集会返回全部？
        return submission, ground_truth
...
def eval_relevant(submission, ground_truth, verbose=True):
    """
    根据 relavent_score 判断是否有答案，并计算 no_answer 的准确率
    Args:
        submission: 预测结果列表
        ground_truth: 真实标签列表
        verbose: 是否输出详细信息
    ...
    for pred, gt in zip(submission, ground_truth):
        # 确保每个样本都有 relavent_score 和 no_answer
        if 'relavent_score' not in pred or 'no_answer' not in gt:
            continue
            
        total += 1
        pred_has_answer = pred['relavent_score'] > 0.5
        gt_has_answer = not gt['no_answer']
        
        # 如果预测没有答案，则将所有预测窗口设为 [0, 0]
        if not pred_has_answer:
            pred['pred_relevant_windows'] = [[0.0, 0.001,0.0] for _ in range(10)]
            
        # 计算准确率
        if pred_has_answer == gt_has_answer:
            correct += 1
            
    acc = correct / total if total > 0 else 0
    if verbose:
        print(f"no_answer accuracy: {acc:.4f}")
...
def vtg_eval(submission, ground_truth, verbose=True, match_number=True):
    ...
    Args:
        ...
        ground_truth: list(dict), each dict is     {
          ...
          "no_answer" : true or false
        }
        verbose: 控制是否输出详细的日志信息,适用于调试和深入了解评估过程。
        match_number: 控制是否强制要求提交和真实标签之间的查询ID完全匹配,确保评估的一致性和准确性。
    ...
    if "pred_relevant_windows" in submission[0]:
        if "relavent_score" in submission[0] and "no_answer" in ground_truth[0]:
            acc,submission = eval_relevant(submission,ground_truth,verbose=verbose)
        ...
        # 如果 acc 存在，则添加到 moment_ret_scores_brief 中
        if "relavent_score" in submission[0] and "no_answer" in ground_truth[0]:
            moment_ret_scores_brief["No-Answer-Acc"] = acc
